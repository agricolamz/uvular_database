...
```{r}
library(DT)
datatable(uvular[,-c(3, 5)], filter = 'top', rownames = FALSE, options = list(pageLength = 20, dom = 'tip'), escape = FALSE)
```

#### bibliography
```{r}
library(DT)
datatable(unique(uvular[,-c(2, 4, 5)]), filter = 'top', rownames = FALSE, options = list(pageLength = 20, autoWidth = FALSE), escape = FALSE)
```
